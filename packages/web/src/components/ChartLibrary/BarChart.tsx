    ...
    <figure className={className} aria-label={title} style={{ margin: 0 }}>
      ...
      {showLegend && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
          {data.map((d, i) => (
            <span key={d.label} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12 }}>
              <span style={{ width: 10, height: 10, borderRadius: 2, background: d.color ?? DEFAULT_COLORS[i % DEFAULT_COLORS.length] }} />
              {d.label}
            </span>
          ))}
        </div>
